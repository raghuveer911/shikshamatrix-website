...
function findFeature(slug: string) {
  return schoolReportCardSoftwareData.features.find((f) => f.slug === slug);
}

export function generateStaticParams() {
  return schoolReportCardSoftwareData.features.filter((f) => f.slug).map((f) => ({ feature: f.slug! }));
}
...
export function generateMetadata({ params }: { params: { feature: string } }): Metadata {
  ...
  return {
    title: `${feature.title} — Report Cards`,
    description: feature.desc,
    alternates: { canonical: `/school-report-card-software/${feature.slug}` },
  };
}
...
export default function Page({ params }: { params: { feature: string } }) {
  ...
  return <FeatureDetailPage parent={schoolReportCardSoftwareData} feature={feature} />;
}
